...
CASE_DESCRIPTION[23] = 'Adversity: compare [n=100, p=200] and [n=200, p=500] (Tiers 1 + 4)'
...
CASE_DESCRIPTION[33] = 'Regression basics: overall baseline + high-dim performance (Tier 6)'
...
CASE_DESCRIPTION[38] = 'Regression scalability: baseline + high-dim for n = 100 (Tier 6)'
CASE_DESCRIPTION[39] = 'Regression scalability: baseline + high-dim for n = 200 (Tier 6)'
...
CASE_DESCRIPTION[44] = 'Regression vs. classification: high-dimensional scenarios (Tiers 2 + 6)'
...
CASE_DESCRIPTION[47] = 'Regression vs. classification: effect of both noise and NaNs (Tiers 3 + 6)'
...
def get_df_cases(df: pd.DataFrame) -> dict[int, pd.DataFrame]:
    ...
    for c, p in zip([4, 5, 6], [1000, 2000, 5000]):
        df_cases[c] = df[
            (df['TIER_ID'] == 2) & (df['SPARSITY'] == DEFAULT_SPARSITY) & (df['N_FEATURES'] == p)
        ]
    ...
    df_cases[16] = df[(df['TIER_ID'].isin([1, 2, 3])) & (df['SPARSITY'] == DEFAULT_SPARSITY)]
    ...
    df_cases[25] = df_tier5[df_tier5['N_FEATURES'] == 500]
    df_cases[26] = df_tier5[df_tier5['SPARSITY'] == 3]
    df_cases[27] = df_tier5[df_tier5['SPARSITY'] == 5]
    df_cases[28] = df_tier5
    ...
    df_cases[33] = df_six[
        (df_six['NOISE_STD'] == BASELINE_NOISE_LEVEL) & (df_six['NAN_RATIO'] == BASELINE_NAN_RATIO)
    ]
    ...
    df_comp = df[(df['TIER_ID'].isin([1, 2, 4, 6])) & (df['SPARSITY'] == DEFAULT_SPARSITY)]
