...
class PerformanceTests:
    ...
    MAX_TOP_FEATURES_FRACTION = (
        0.5  # top_n_features should not exceed 50% of n_features
    )
    ...
    def __init__(
        ...
        self.mu_array = np.array(
            history["mu"]
        )  # shape: [n_iterations, n_components, n_features]
    ...
    def test_top_feature_ordering(
        ...
        top_n_features = min(
            top_n_features, int(self.MAX_TOP_FEATURES_FRACTION * self.n_features)
        )
        ...
        for component in range(self.n_components):
            ...
            first_top_indices = np.argsort(np.abs(first_mu[component]))[
                -top_n_features:
            ]
            ...
            component_details.append(
                {
                    ...
                    "jaccard_similarity": intersection_count
                    / len(first_set.union(last_set)),
                }
            )
        ...
        if status == "FAILED":
            message = (
                ...
                + self._format_component_summary(
                    n_failed_components, n_warning_components
                )
            )
        elif status == "WARNING":
            message = (
    ...
    def test_sparsity_gap(
        ...
        for component in range(self.n_components):
            ...
            relative_threshold_boundary = (
                difference_coefficient_boundary * selected_diff
            )
        ...
        if status == "FAILED":
            ...
            message = "All components have good separation between selected and not selected features "

        test_result = {
            "test_name": test_name,
            "status": status,
            "message": message
            + " WARNING: WORK IN PROGRESS! RESULTS MAY BE TOO PESIMISTIC!",
            "details": {
    ...
    def _determine_test_status(
        ...
        """
        fail_score = (
            self.FAIL_WEIGHT * n_failed_components
            + self.WARNING_WEIGHT * n_warning_components
        )
        test_fail_threshold = self.n_components * self.COMPONENT_FAIL_THRESHOLD_RATIO
        test_warn_threshold = (
            test_fail_threshold * self.COMPONENT_WARNING_THRESHOLD_RATIO
        )
    ...
    def _display_sparsity_gap(self, details: Dict[str, Any]) -> None:
        ...
        for comp_detail in details["component_details"]:
            ...
            if component_status == "FAILED":
                ...
                display(
                    Markdown(
                        "  - Good separation of selected and not selected features."
                    )
                )
            ...
            if component_status in ["FAILED", "WARNING"]:
                selected_values = [
                    np.round(v, 4) for v in comp_detail["top_features_values"]
                ]
                other_values = [
                    np.round(v, 4) for v in comp_detail["other_features_values"]
                ]
                display(
                    Markdown(
