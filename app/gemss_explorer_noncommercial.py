...
def _(
    ...
    component_features['all_selected_features'] = unique_features_found[selected_solution_type]
    ...
    for component_name, feature_list in component_features.items():
        ...
        tabpfn_results[component_name] = tabpfn_evaluate(
            ...
            random_state=42,
            verbose=False,
            explain=(checkbox_shap.value if component_name != 'all_selected_features' else False),
            shap_sample_size=50,
        )
    ...
    for _comp, _result in tabpfn_results.items():
        if 'shap_explanations' in _result:
            ...
            _eval_displays.append(
                mo.md(f'**Shapley values:** feature importances in the model from {_comp}')
            )
    ...
    if save_results:
        ...
        if 'shap_explanations' in _result:
            ...
            _eval_displays.append(
                mo.md(f'📁 **Shapley values saved to:** `{shap_path.split("/")[-1]}`<br><br>'),
            )
